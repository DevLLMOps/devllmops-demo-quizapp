...
if [ -z "$SCAN_DIRS" ]; then
  ...
  echo -e "${YELLOW}  Example: SCAN_DIRS=\"./scripts,./app\" ./scripts/validate-envs.sh${NC}"
  TOTAL_USED=0
else
...
if [ -d "$K8S_DIR" ]; then
  ...
  echo -e "${YELLOW}  Directory ${K8S_DIR} not found, skipping...${NC}"
  touch "$K8S_VARS_FILE"
  touch "$K8S_VALUES_VARS_FILE"
  TOTAL_K8S=0
fi
